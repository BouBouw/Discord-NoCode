...
const DISCORD_ERROR_MSG_KEYS: Record<string, 'discordFailed' | 'discordCancelled'> = {
  discord_failed: 'discordFailed',
  discord_cancelled: 'discordCancelled',
};
...
export default function LoginPage() {
  ...
  const displayError = error || (oauthError ? (DISCORD_ERROR_MSG_KEYS[oauthError] ? t.auth[DISCORD_ERROR_MSG_KEYS[oauthError]] : t.auth.loginError) : '');
    ...
    <div className="min-h-screen flex" style={{ ...thV, backgroundColor: 'var(--t-bg)' }}>
      ...
      <div className="flex-1 flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          <div className="mb-8">
            <h1 className="text-2xl font-bold" style={{ color: 'var(--t-tx)' }}>{t.auth.login}</h1>
            <p className="text-sm mt-1" style={{ color: 'var(--t-sub)' }}>{t.auth.loginWelcome}</p>
          </div>

          {displayError && (
            <div
              className="px-4 py-3 rounded-xl mb-5 flex items-center gap-2.5 text-sm"
              style={{ backgroundColor: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', color: '#f87171' }} // semantic error
            >
